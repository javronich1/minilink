...
# Create app with lifespan (no deprecated on_event)
...
def create_link(payload: LinkCreate, session: Session = Depends(get_session)):
    ...
    # choose short code (custom or random), ensure uniqueness
...
def redirect(code: str, session: Session = Depends(get_session)):
    ...
    # 307 preserves method semantics
...
def create_form(
    ...
    label: Optional[str] = Form(None),   # Python 3.9 compatible Optional
